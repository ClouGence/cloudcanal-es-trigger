...
import com.clougence.cloudcanal.es7.trigger.CcEs7IdxTriggerPlugin;
...
public class Es7ClientConn {
    ...
    public void addHostSettingConsumer(ClusterSettings settings) {
        String hosts = settings.get(CcEs7IdxTriggerPlugin.triggerIdxHost);
        configInMem.put(EsTriggerConstant.TRIGGER_IDX_HOST_KEY, hosts);

        String user = settings.get(CcEs7IdxTriggerPlugin.triggerIdxUser);
        configInMem.put(EsTriggerConstant.TRIGGER_IDX_USER_KEY, user);

        String password = settings.get(CcEs7IdxTriggerPlugin.triggerIdxPassword);
        configInMem.put(EsTriggerConstant.TRIGGER_IDX_PASSWD_KEY, password);

        log.info("Init host settings,hosts:" + hosts + ",user:" + user);

        settings.addSettingsUpdateConsumer(CcEs7IdxTriggerPlugin.triggerIdxHost, s -> configChange(EsTriggerConstant.TRIGGER_IDX_HOST_KEY, s));
        settings.addSettingsUpdateConsumer(CcEs7IdxTriggerPlugin.triggerIdxUser, s -> configChange(EsTriggerConstant.TRIGGER_IDX_USER_KEY, s));
        settings.addSettingsUpdateConsumer(CcEs7IdxTriggerPlugin.triggerIdxPassword, s -> configChange(EsTriggerConstant.TRIGGER_IDX_PASSWD_KEY, s));
