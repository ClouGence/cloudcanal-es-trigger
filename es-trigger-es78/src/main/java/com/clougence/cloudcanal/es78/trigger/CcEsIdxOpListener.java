...
public class CcEsIdxOpListener implements IndexingOperationListener, ComponentLifeCycle, Consumer<Boolean> {
    ...
    public void postDelete(ShardId shardId, Engine.Delete delete, Engine.DeleteResult result) {
        try {
            ...
            //            ccEsTriggerIdxWriter.insertTriggerIdx(indexName, DataOp.DELETE, delId, null, 0, delete.getIfSeqNo());
        } catch (Exception e) {
    ...
    public void postIndex(ShardId shardId, Engine.Index index, Engine.IndexResult result) {
        try {
            ...
            if (result.isCreated()) {
                log.info("[INSERT] " + indexName + " data,seq:" + index.getIfSeqNo() + ",ts:" + index.getAutoGeneratedIdTimestamp() + ",ptm:" + index.getIfPrimaryTerm() + ",data:"
                         + doc.source().utf8ToString());
                //                ccEsTriggerIdxWriter.insertTriggerIdx(indexName, DataOp.INSERT, index.id(), doc, index.getAutoGeneratedIdTimestamp(), index.getIfSeqNo());
            } else {
                log.info("[UPDATE] " + indexName + " data,seq:" + index.getIfSeqNo() + ",ts:" + index.getAutoGeneratedIdTimestamp() + ",ptm:" + index.getIfPrimaryTerm() + ",data:"
                         + doc.source().utf8ToString());
                //                ccEsTriggerIdxWriter.insertTriggerIdx(indexName, DataOp.UPDATE, index.id(), doc, index.getAutoGeneratedIdTimestamp(), index.getIfSeqNo());
            }
        } catch (Exception e) {
