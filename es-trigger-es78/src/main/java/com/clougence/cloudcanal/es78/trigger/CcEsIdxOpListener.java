    ...
    public void postIndex(ShardId shardId, Engine.Index index, Engine.IndexResult result) {
        try {
            ...
            if (result.isCreated()) {
                ...
                ccEsTriggerIdxWriter.insertTriggerIdx(indexName, TriggerEventType.INSERT, index.id(), doc);
            } else {
                if (log.isDebugEnabled()) {
                    log.debug("[UPDATE] " + indexName + " data,seq:" + index.getIfSeqNo() + ",ts:" + index.getAutoGeneratedIdTimestamp() + ",ptm:" + index.getIfPrimaryTerm()
                              + ",data:" + doc.source().utf8ToString());
                }
                ccEsTriggerIdxWriter.insertTriggerIdx(indexName, TriggerEventType.UPDATE, index.id(), doc);
            }
        } catch (Exception e) {
