...
public class Es7ClientHelper {
    ...
    public static RestHighLevelClient generateEsClient(String hostListStr, String userName, String password, boolean isHttpsEnabled, boolean caCertificated,
                                                       String securityFileUrl) {
        List<HttpHost> hostList = genEsHttpHostList(hostListStr, isHttpsEnabled);
        RestClientBuilder builder = RestClient.builder(hostList.toArray(new HttpHost[] {}));

        builder.setHttpClientConfigCallback(genHttpClientConfigCallBack(userName, password, caCertificated, securityFileUrl, null, null, null));
